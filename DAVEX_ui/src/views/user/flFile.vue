  ...
  <div>
    ...
    <div class="form-container" style="margin-top: 10px">
      ...
      >
        ...
        >
          <template #trigger>
            <el-button class="default-button" style="margin-right: 10px;">选择联邦学习文件</el-button>
          </template>
...
import { getFlList } from '../../api/fLearning.js'
...
const uploadMethod = async () => {
  try {
    ...
    if (res.data.body.code == 1) {
      mpcSuccessMessage.value = `联邦学习文件上传成功`
    ...
    console.error('Failed to create 联邦学习 task:', error)
  }
}
...
//查看联邦学习列表
const flFileList = ref([])
const getMpcListMethod = async () => {
  try {
    const res = await getFlList()
    flFileList.value = res.data.body.data
    console.log(flFileList.value)
  } catch (error) {
